$(function() {
	...
	// Merge Items
	$("#merge a.name1").each(function(i) {
		var diff = $(this).parent().parent().find(".name2:first").attr('data-id');
		$(this).attr('data-diff', diff);
	});
	$("#merge a").click(function(e) {
		e.preventDefault();
		var $obj = $(this);
		var table = $("#merge").attr('data-source');
		var source = $obj.attr('data-id');
		var diff = $obj.attr('data-diff');
		$.ajax({
			url: '?view=callback&task=merge&table=' + table + '&source=' + source + '&diff=' + diff
			, type: 'get'
			, success: function(msg) {
				if (msg != 'done') {
					alert(msg);
				} else {
					if ($obj.hasClass('name1')) {
						$obj.parent().next().fadeOut();
						$obj.parent().next().next().fadeOut(function() {
							$obj.parent().attr('colspan', 3);
						});
					} else {
						$obj.parent().prev().fadeOut('fast');
						$obj.parent().prev().prev().fadeOut('fast', function() {
							$obj.parent().attr('colspan', 3);
						});
					}
				}
	...
	$("#enable-merge").click(function(e) {
		$(this).parent().parent().find("select").prop('disabled', false);
		$(".chosen").chosen('destroy');
		$('input[name="form-task"]').val('merge');
		initChosen();
		$(this).remove();
		e.preventDefault();
	});
	...
	// $("#musics").dataTable({
		// "bProcessing": true,
		// "sAjaxSource": '?view=callback&table=items'
    // });
	
	$("*[data-toggle=tooltip]").tooltip(); // Init tooltip
	
	$(".search-tools-toggle").click(function(e) { // Search Tools
		if (!$(".search-tools").is(':visible')) {
			$(this).find("i").removeClass("icon-angle-left").addClass("icon-angle-right");
			$(".search-tools").slideDown(400);
		} else {
			$(this).find("i").removeClass("icon-angle-right").addClass("icon-angle-left");
			$(".search-tools").slideUp(400);
		}
		e.preventDefault();
	});

	$(".toggle-logout").click(function(e) {
		if (!$("#logout").hasClass('open')) {
			$(this).animate({'top': '+=37px'}).find("i").removeClass('icon-chevron-down').addClass('icon-chevron-up');
			$("#logout").slideDown().addClass('open');
		} else {
			$(this).animate({'top': '-=37px'}).find("i").removeClass('icon-chevron-up').addClass('icon-chevron-down');
			$("#logout").slideUp().removeClass('open');
		}
		e.preventDefault();
	});
	...
	$("#live-search").keyup(function(e){
		...
		if ($(this).val().length > 2 && e.key != 'enter') {
			...
			$.ajax({
				url: '?view=callback&task=' + task + '&keyword=' + $(this).val()
				, type: 'get'
			...
			$(this).addClass('loading');
			$.ajax({
				url: '?view=callback&task=' + task + '&keyword='
				, type: 'get'
				, success: function (data) {
					$("#live-search").removeClass('loading');
					liveSearch.handle(data);
	...
	var liveSearch = {
		handle: function(data) {
			this.prepare();
			parts = data.split('$$$');
			this.pagination(parts[2], parts[0]);
			this.items(parts[1]);
		...
		prepare: function() {
			$("#items").find("table").find("tbody").empty();
			$("#pagination").empty();
		},
		pagination: function(count, data) {
			// alert(count);
		...
		items: function(data) {
			$("#items").find("table").find("tbody").html(data);
		}
	}
	
	function manipulatePagination() {
		$("#pagination").find("li:not(.disabled)").find("a").each(function(i) {
			// هان
			var href = $(this).attr('href');
			// if (href.indexOf('?view=callback') != -1)
				// href = href.replace('?view=callback', '?');
			var nHref = href + '&keyword=' + $("#live-search").val();
			// console.log(nHref);
			$(this).attr('href', href + '&keyword=' + $("#live-search").val());
		});
		ajaxPagination();
	}
	var keyword = queryString.keyword;
	if (typeof keyword != "undefined" && keyword != "") {
		$("#live-search").val(decodeURIComponent((keyword + '').replace(/\+/g, '%20')));
		manipulatePagination();
	}
	...
	function ajaxPagination() {
		$("#pagination a").click(function(e) {
			if (!$(this).parent().hasClass('disabled')) {
				var href = $(this).attr('href');
				if (href.indexOf('callback') != -1) {
					var keyword = $("#live-search").val();
					var limitstart = (parseQueryString(href).start == '0' || typeof parseQueryString(href).start == 'undefined') ? 0 : parseQueryString(href).start;
					if (keyword.length > 2) {
						var source = $("#live-search").attr("data-source");
						var task = (source == 'items') ? 'search' : source;
						$.ajax({
							url: '?view=callback&task=' + task + '&keyword=' + keyword + '&limitstart=' + limitstart
							, type: 'get'
							, success: function (data) {
								$("#live-search").removeClass('loading');
								liveSearch.handle(data);
	...
	var modals = {
		...
		items: {
			...
			destroyPlayer: function() {
				$modal.find(".modal-footer > div").remove();
				$modal.find(".modal-footer").append('<div id="audio-player" class="pull-left"></div>');
			}
		},
		singers: {
			edit: function(type, id) {
			...
			details: function(type, id) {
				...
				modals.items.destroyPlayer(); // Temporary
			}
		},
		genres: {
			edit: function(type, id) {
		...
		composers: {
			...
			details: function(type, id) {
				$modal.modal('toggle');
				$.ajax({
					url: '?view=composerdetails&id=' + id
					, method: 'GET'
					, success: function (data) {
						$modal.find(".modal-body").html(data);
						$("*[data-toggle=tooltip]").tooltip();
		...
		musicians: {
			...
			details: function(type, id) {
				$modal.modal('toggle');
				$.ajax({
					url: '?view=musiciandetails&id=' + id
					, method: 'GET'
					, success: function (data) {
						$modal.find(".modal-body").html(data);
						$("*[data-toggle=tooltip]").tooltip();
	...
	function initTools () {
		$(".tool").click(function(e) {
			var task = $(this).attr('data-do');
			var type = $(this).parents().find("table").attr('id');
			var id = $(this).parent().parent().attr('data-id');
			var obj = $(this).parent().parent();
			if (task == 'delete') {
				var r = confirm('Are you sure you want to ' + task + ' [' + id + '] from ' + type + '?');
				if (r == true) {
					tools(task, type, id, obj);
				}
			} else {
				tools(task, type, id, obj);
			}
	...
	function tools(task, type, id, obj) {
		$.ajax({
			url: '?view=tools&task=' + task + '&type=' + type + '&id=' + id
			, method: 'GET'
	...
	function handleModal() {
		$("#items a[data-do]").click(function(e) {
			// var type = 'items';
			var type = $("#items > table").attr("id");
			var task = $(this).attr('data-do');
			var item = $(this).parents("tr").attr('data-id');
			modals.setTitle($(this).parents("tr").find("td.fa > span > a").text());
			if (task == 'details') {
				switch (type) {
					default:
					case 'musics':
						modals.items.details(type, item);
						break;
					case 'singers':
						modals.singers.details(type, item);
						break;
					case 'genres':
						modals.genres.details(type, item);
						break;
					case 'poets':
					case 'lyricists':
						modals.poets.details(type, item);
						break;
					case 'composers':
						modals.composers.details(type, item);
						break;
					case 'musicians':
						modals.musicians.details(type, item);
						break;
					case 'albums':
						modals.albums.details(type, item);
						break;
				}
			}
			if (task != 'edit')
	...
	$modal.on('hide', function() {
		modals.items.destroyPlayer();
	});
	
	if ($("#detail").length) {
		$("#detail").find("select").each(function(i) {
			$(this).parent().append('<small></small>');
		});
		$("#detail").find("select").each(function(i) {
			var text = $(this).find("option:selected").text();
			$(this).parent().find('small').text(text);
		});
		$("#detail").find("select").change(function() {
			var text = $(this).find("option:selected").text();
			$(this).parent().find('small').text(text);
		});
	}
